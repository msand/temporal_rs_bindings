...
function resolveEraForCalendar(calId, napiYear, napiEra, napiEraYear) {
  if (calId === 'ethioaa') {
    // ethioaa always uses "aa" era; the NAPI already returns correct AA eraYear
...
class PlainDate {
  ...
  toPlainYearMonth() {
    ...
    const calId = getRealCalendarId(this);
    const cal = toNapiCalendar(calId);
    // Use ISO year from toString() to avoid calendar-year confusion
    const isoDate = _extractISOFromNapiDT(this._inner);
    return wrapPlainYearMonth(call(() => new NapiPlainYearMonth(isoDate.year, isoDate.month, cal)), calId);
  }
