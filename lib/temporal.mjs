...
function toNapiZonedDateTime(arg) {
  ...
  if (typeof arg === 'object' && arg !== null) {
    ...
    // Use PlainDate to properly constrain day
    const cal = toNapiCalendar(arg.calendar);
    try {
      const pd = call(() => new NapiPlainDate(year, month, day, cal));
      day = pd.day;
      month = pd.month;
    } catch {
      // Try max valid day
      for (let d = 28; d <= 31; d++) {
        try { call(() => new NapiPlainDate(year, month, d, cal)); day = d; } catch { break; }
      }
    }
    let str = `${padYear(year)}-${pad2(month)}-${pad2(day)}T${pad2(hour)}:${pad2(minute)}:${pad2(second)}`;
...
class ZonedDateTime {
  ...
  static from(arg, options) {
    ...
    if (typeof arg === 'object' && arg !== null) {
      ...
      // Use PlainDate to properly constrain the day for the given calendar/month
      const cal = toNapiCalendar(arg.calendar);
      let isoDay = day;
      try {
        const pd = call(() => new NapiPlainDate(year, month, day, cal));
        isoDay = pd.day;
        // Re-read constrained month too
        month = pd.month;
      } catch (e) {
        if (overflow === 'Reject') throw wrapError(e);
        // If constrain mode and day still too large, try to find max day
        try {
          // Try day 28 (safe for all months) then work up
          const pd28 = call(() => new NapiPlainDate(year, month, 28, cal));
          for (let d = 29; d <= 31; d++) {
            try { call(() => new NapiPlainDate(year, month, d, cal)); isoDay = d; } catch { break; }
          }
        } catch { isoDay = 28; }
      }
      let str = `${padYear(year)}-${pad2(month)}-${pad2(isoDay)}T${pad2(hour)}:${pad2(minute)}:${pad2(second)}`;
