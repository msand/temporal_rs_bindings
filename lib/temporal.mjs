...
class ZonedDateTime {
  ...
  static from(arg, options) {
    if (typeof arg === 'string') {
      rejectTooManyFractionalSeconds(arg);
      validateOptions(options);
      const offsetOpt = options ? options.offset : undefined;
      const offsetMode = offsetOpt !== undefined ? String(offsetOpt) : 'reject';
      // Validate offset option
      if (offsetOpt !== undefined && !['use', 'prefer', 'ignore', 'reject'].includes(offsetMode)) {
      ...
      if (offsetMode === 'ignore') {
        ...
        // This tells the NAPI to interpret the datetime as wall time in the timezone
