...
class ZonedDateTime {
  constructor(epochNanoseconds, timeZone, calendar) {
    if (epochNanoseconds instanceof NapiZonedDateTime) {
      ...
      try {
        ...
        // create an inner at the boundary with the correct offset for the timezone.
        // For fixed-offset timezones, construct a valid string at the boundary.
        ...
        if (isFixedOffset) {
          ...
          // For IANA timezones, try UTC string approach
  ...
  static from(arg, options) {
    if (typeof arg === 'string') {
      ...
      validateOptions(options);
      // Read option values with proper type conversion (read each option only once)
      // Per spec: read disambiguation, offset, overflow in alphabetical order
      let disambiguation = 'compatible';
      if (options && options.disambiguation !== undefined) {
        disambiguation = toStringOption(options.disambiguation);
    ...
    if (typeof arg === 'object' && arg !== null) {
      ...
      // Read options only once each to avoid double property access
      let _pbDisambiguation = 'compatible';
      if (options && options.disambiguation !== undefined) {
        _pbDisambiguation = toStringOption(options.disambiguation);
  ...
  with(fields, options) {
    ...
    const withOptions = options ? Object.assign({}, options) : {};
    if (withOptions.offset === undefined) withOptions.offset = 'prefer';
