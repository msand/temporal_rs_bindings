...
const CALENDAR_ISO_OFFSETS = {
  buddhist: -543, roc: 1911, japanese: 0, gregory: 0, iso8601: 0,
  coptic: 284, ethiopic: 8, ethioaa: -5492, indian: 78, persian: 621,
  hebrew: -3760, 'islamic-civil': 579, 'islamic-tbla': 579, 'islamic-umalqura': 579,
  chinese: 0, dangi: 0,
};
...
function addWithOverflow(inner, dur, overflow, op, wrapFn) {
  if (overflow === 'Reject') {
    ...
    if (hasYearMonth) {
      ...
      const dateOnlyDur = call(() => new NapiDuration(
        sign * dur.years, sign * dur.months, sign * dur.weeks, 0, 0, 0, 0, 0, 0, 0
      ));
...
class PlainDateTime {
  ...
  with(fields, options) {
    ...
    // Check at least one recognized field was provided
    if (_day === undefined && _hour === undefined && _microsecond === undefined &&
        _millisecond === undefined && _minute === undefined && _month === undefined &&
        _monthCode === undefined && _nanosecond === undefined && _second === undefined &&
        _year === undefined && !_hasEra && !_hasEraYear) {
      throw new TypeError('At least one recognized property must be provided');
    }
    ...
    // Determine which monthCode is in effect for leap-month validation
    const effectiveMonthCodeDT = _monthCode !== undefined ? monthCodeStr : (_month === undefined ? this.monthCode : undefined);
    // In reject mode, a leap monthCode in a non-leap year is an error
...
class ZonedDateTime {
  ...
  with(fields, options) {
    ...
    if (_day === undefined && _hour === undefined && _microsecond === undefined &&
        _millisecond === undefined && _minute === undefined && _month === undefined &&
        _monthCode === undefined && _nanosecond === undefined && _offset === undefined &&
        _second === undefined && _year === undefined && !_hasEra && !_hasEraYear) {
      throw new TypeError('At least one recognized property must be provided');
    }
    const merged = { day: dayVal, hour: hourVal, microsecond: microsecondVal, millisecond: millisecondVal,
      minute: minuteVal, nanosecond: nanosecondVal, second: secondVal, offset: offsetStr, year, era, eraYear };
    ...
    const effectiveMonthCodeZDT = _monthCode !== undefined ? monthCodeStr : (_month === undefined ? this.monthCode : undefined);
  ...
  toString(options) {
    ...
    const displayCalendar = cnStr !== undefined ? (() => { const m = DISPLAY_CALENDAR_MAP[cnStr]; if (!m) throw new RangeError(`Invalid calendarName option: ${cnStr}`); return m; })() : undefined;
    const displayOffset = offStr !== undefined ? (() => { const m = DISPLAY_OFFSET_MAP[offStr]; if (!m) throw new RangeError(`Invalid offset option: ${offStr}`); return m; })() : undefined;
    const roundingMode = rmStr !== undefined ? (() => { const m = ROUNDING_MODE_MAP[rmStr]; if (!m) throw new RangeError(`Invalid rounding mode: ${rmStr}`); return m; })() : 'Trunc';
    let smallestUnit;
    if (suStr !== undefined) {
    ...
    const displayTimeZone = tznStr !== undefined ? (() => { const m = DISPLAY_TIMEZONE_MAP[tznStr]; if (!m) throw new RangeError(`Invalid timeZoneName option: ${tznStr}`); return m; })() : undefined;
...
class PlainYearMonth {
  ...
  static from(arg, options) {
    ...
    if (typeof arg === 'object' && arg !== null) {
      ...
      function validateYearMonthRange(ymResult) {
        ...
        if (!m) return;
        const rY = parseInt(m[1], 10), rM = parseInt(m[2], 10), rD = parseInt(m[3], 10);
      ...
      if (calId !== 'iso8601') {
        try {
          ...
          if (day1Iso.isoYear < -271821 ||
              (day1Iso.isoYear === -271821 && (day1Iso.isoMonth < 4 ||
                (day1Iso.isoMonth === 4 && day1Iso.isoDay < 19)))) {
            throw new RangeError('PlainYearMonth outside representable range');
          }
          ...
          if (outerErr instanceof RangeError && outerErr.message === 'PlainYearMonth outside representable range') throw outerErr;
              ...
              try {
                ...
                if (probe.year === calYear && probe.month === constrainedMonth) {
                  ...
                  if (iy === 275760) {
                    ...
                    } catch { /* day beyond PlainDate range but ok for PlainYearMonth */ }
                  }
                  try { validateYearMonthRange(probe); } catch { continue; }
  ...
  with(fields, options) {
    ...
    // Determine which monthCode is in effect for leap-month validation
    const effectiveMonthCodeYM = _monthCode !== undefined ? monthCodeStr : (_month === undefined ? this.monthCode : undefined);
    ...
    if (typeof arg === 'object' && arg !== null) {
      ...
      if (hasMonthCode) {
        ...
        if (yearVal !== undefined && monthCodeStr.endsWith('L')) {
          ...
          if (!yearHasLeap) {
            if (overflow === 'Reject') {
              throw new RangeError(`monthCode ${monthCodeStr} does not exist in year ${yearVal} for ${mdFromCalId} calendar`);
            }
...
function _temporalToISOFields(temporalObj) {
  ...
  if (inner instanceof NapiPlainDateTime) {
    ...
    if (m) return { year: parseInt(m[1], 10), month: parseInt(m[2], 10), day: parseInt(m[3], 10), hour: parseInt(m[4], 10), minute: parseInt(m[5], 10), second: parseInt(m[6], 10) };
    return { year: inner.year, month: inner.month, day: inner.day, hour: inner.hour, minute: inner.minute, second: inner.second };
  }
  if (inner instanceof NapiPlainDate) {
    const str = inner.toString();
    const m = str.match(/^(-?\d+|\+?\d+)-(\d{2})-(\d{2})/);
    if (m) return { year: parseInt(m[1], 10), month: parseInt(m[2], 10), day: parseInt(m[3], 10), hour: 12, minute: 0, second: 0 };
  ...
  if (inner instanceof NapiPlainYearMonth) {
    ...
    if (m3) return { year: parseInt(m3[1], 10), month: parseInt(m3[2], 10), day: parseInt(m3[3], 10), hour: 12, minute: 0, second: 0 };
  ...
  if (inner instanceof NapiPlainMonthDay) {
    ...
    if (m) {
      if (m.length === 4) return { year: parseInt(m[1], 10), month: parseInt(m[2], 10), day: parseInt(m[3], 10), hour: 12, minute: 0, second: 0 };
...
function _formatExtremeTemporalParts(dtf, isoFields) {
  ...
  const shiftedYearStr = String(shiftedYear);
  const actualYearStr = year < 0 ? String(year) : String(year);
  ...
  if (isWallClock) {
    ...
    if (inner instanceof NapiPlainDate) {
      ...
      if (opts.dateStyle) {
        ...
        delete opts.dateStyle;
        if (ds === 'full' || ds === 'long') { opts.month = 'long'; opts.year = 'numeric'; }
        else if (ds === 'medium') { opts.month = 'short'; opts.year = 'numeric'; }
        else if (ds === 'short') { opts.month = 'numeric'; opts.year = '2-digit'; }
      }
      ...
      if (opts.dateStyle) {
        const ds = opts.dateStyle;
        delete opts.dateStyle;
        if (ds === 'full' || ds === 'long') { opts.month = 'long'; opts.day = 'numeric'; }
        else if (ds === 'medium') { opts.month = 'short'; opts.day = 'numeric'; }
        else if (ds === 'short') { opts.month = 'numeric'; opts.day = 'numeric'; }
      }
    }
