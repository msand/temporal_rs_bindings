...
class PlainDate {
  ...
  toLocaleString(locales, options) {
    ...
    if (m) {
      ...
      const opts = options ? Object.assign({}, options, { timeZone: 'UTC' }) : { timeZone: 'UTC' };
  ...
  static from(arg, options) {
    if (typeof arg === 'string') {
      ...
      if (offsetMode === 'use') {
        ...
        return _zdtFromStringWithOffset(cleanArg, 'use');
      }
      try {
...
function _getTemporalDtf(dtf, temporalObj) {
  ...
  if (isWallClock) {
    ...
    for (const key of ['locale', 'calendar', 'numberingSystem', 'year', 'month', 'day',
      ...
      'dayPeriod', 'dateStyle', 'timeStyle', 'hour12', 'hourCycle']) {
...
function _rejectUnsupportedTemporalDTF(arg, methodName, dtf) {
  if (arg !== undefined && typeof arg === 'object' && arg !== null && '_inner' in arg) {
    ...
    // Per spec: check for overlapping options
    if (dtf) {
      ...
      if (inner instanceof NapiPlainTime && (hasDateStyle || resolved.year || resolved.month || resolved.day || resolved.weekday)) {
        // PlainTime with date-related options
        if (hasDateStyle) throw new TypeError(`dateStyle is not compatible with Temporal.PlainTime`);
      }
      if (inner instanceof NapiPlainDate && hasTimeStyle) {
        throw new TypeError(`timeStyle is not compatible with Temporal.PlainDate`);
      }
      if (inner instanceof NapiPlainYearMonth && hasTimeStyle) {
        throw new TypeError(`timeStyle is not compatible with Temporal.PlainYearMonth`);
      }
      if (inner instanceof NapiPlainMonthDay && hasTimeStyle) {
        throw new TypeError(`timeStyle is not compatible with Temporal.PlainMonthDay`);
      }
    }
  }
}
...
if (_origFormatRange) {
  Intl.DateTimeFormat.prototype.formatRange = function formatRange(a, b) {
    ...
    // Per spec: both args must be the same Temporal type
