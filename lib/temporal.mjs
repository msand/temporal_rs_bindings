...
function _parseDurationForInstant(arg) {
  ...
  // For Duration wrapper instances
  if (arg && _wrapperSet.has(arg) && arg._inner instanceof NapiDuration) {
  ...
  // Property bag: read fields directly as numbers, convert to BigInt before NAPI
  if (typeof arg === 'object' && arg !== null) {
    ...
    const yv = toIntegerIfIntegral(years);
    const mov = toIntegerIfIntegral(months);
    const wv = toIntegerIfIntegral(weeks);
    const dv = toIntegerIfIntegral(days);
    if (yv || mov || wv || dv) {
      throw new RangeError('Instant.add/subtract does not accept date components');
    }
    const hv = toIntegerIfIntegral(hours) || 0;
    const mv = toIntegerIfIntegral(minutes) || 0;
    const sv = toIntegerIfIntegral(seconds) || 0;
    const msv = toIntegerIfIntegral(milliseconds) || 0;
    const usv = toIntegerIfIntegral(microseconds) || 0;
    const nsv = toIntegerIfIntegral(nanoseconds) || 0;
    // Convert to BigInt for precision
    const totalNs = BigInt(hv) * 3600000000000n +
                    BigInt(mv) * 60000000000n +
                    BigInt(sv) * 1000000000n +
                    BigInt(msv) * 1000000n +
                    BigInt(usv) * 1000n +
                    BigInt(nsv);
    // Also create a NAPI duration for validation (with clamped values for NAPI compatibility)
    const dur = call(() => new NapiDuration(0, 0, 0, 0, hv, mv, sv, msv, usv, nsv));
    return { dur, totalNs };
  }
...
function _resolveLocalToEpochMs(isoYear, isoMonth, isoDay, hour, minute, second, ms, tzId, disambiguation) {
  if (tzId === 'UTC' || /^[+-]\d{2}:\d{2}$/.test(tzId)) {
    ...
    if (tzId === 'UTC') {
      return { epochMs: d.getTime(), offsetStr: '+00:00' };
    }
    ...
    return { epochMs: d.getTime() - offsetMs, offsetStr: tzId };
  }
...
function toNapiZonedDateTime(arg) {
  ...
  if (typeof arg === 'object' && arg !== null) {
    ...
    // Read each property exactly once to avoid double valueOf calls
    ...
    const millisecondVal = toInteger(arg.millisecond);
    const microsecondVal = toInteger(arg.microsecond);
    const nanosecondVal = toInteger(arg.nanosecond);
    ...
    if (arg.offset !== undefined && typeof arg.offset === 'string' && isValidOffsetString(arg.offset)) {
      const providedOffsetNs = parseOffsetStringToNs(arg.offset);
      if (providedOffsetNs !== undefined) {
        ...
        if (!offsetMatches) {
          throw new RangeError(`Offset ${arg.offset} does not match the time zone offset for ${tz.id}`);
        }
      }
    }
...
class PlainDate {
  ...
  with(fields, options) {
    ...
    // Per spec: for ISO calendar, era/eraYear are silently ignored (not part of the field set)
    // For non-ISO calendars without eras (chinese, dangi, etc.), era/eraYear should throw TypeError
  ...
  toZonedDateTime(item) {
    ...
    if (typeof item === 'string') {
      ...
      const zdtStr = baseStr + 'T00:00:00[' + tz.id + ']' + calAnnotation;
  ...
  static from(arg, options) {
    ...
    if (typeof arg === 'object' && arg !== null) {
      ...
      if (overflow !== 'Reject' && !ISO_MONTH_ALIGNED_CALENDARS.has(calId)) {
        const maxM = _getMaxMonthForCalendarYear(calId, fields.year);
        constrainedMonth = Math.max(1, Math.min(month, maxM));
      }
      const iso = calendarDateToISO(fields.year, constrainedMonth, day, calId);
...
class ZonedDateTime {
  ...
  with(fields, options) {
    ...
    if (_fieldsMonth !== undefined && fields.monthCode !== undefined) {
      ...
      // Extract month number without calendar validation
      ...
      if (mcMatch) {
        ...
        _pendingMonthCodeValidation = { monthCode: mc, calId, year: merged.year };
      } else {
        // Let monthCodeToMonth handle the error
        merged.month = monthCodeToMonth(fields.monthCode, undefined, merged.year);
      }
    } else {
...
class PlainYearMonth {
  ...
  static from(arg, options) {
    ...
    if (typeof arg === 'object' && arg !== null) {
      ...
      const calId = getCalendarId(arg.calendar);
      const cal = toNapiCalendar(arg.calendar);
      // Per spec: fetch month/monthCode BEFORE year (CalendarResolveFields order)
      const rawMonth = arg.month;
      const rawMonthCode = arg.monthCode;
      ...
      resolveEraYear(fields, calId);
      if (fields.year === undefined) throw new TypeError('Required property year is missing or undefined');
      const month = resolveMonth({ month: rawMonth, monthCode: rawMonthCode }, calId, fields.year);
...
class PlainMonthDay {
  ...
  static from(arg, options) {
    ...
    if (typeof arg === 'object' && arg !== null) {
      ...
      const targetMonthCode = effectiveMonthCode || (hasMonthCode ? arg.monthCode : undefined);
      const isLeapMonthCode = targetMonthCode && targetMonthCode.endsWith('L');
      ...
      for (const c of candidates) {
        ...
        if (tryIso.isoYear > 1972) continue;
        // Verify monthCode
        if (targetMonthCode) {
          try {
      ...
      if (!bestIso) {
        // Fallback: find any candidate even if ISO year > 1972, then constrain
        for (const c of candidates) {
